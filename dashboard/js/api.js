/* ============================================================
   NESTeq V3 Dashboard — API Layer
   Reads endpoints from config.js (loaded before this file)
   ============================================================ */

const API = {
  AI_MIND: CONFIG.AI_MIND_URL,
  FOX_MIND: CONFIG.FOX_MIND_URL,
...
const Workers = {
  ENDPOINTS: Object.entries(CONFIG.WORKERS || {}).map(([name, url]) => ({
    name, url: url.replace('/health', ''), desc: name
  })),
  ...
  async getGarminSyncStatus() {
    try {
      const controller = new AbortController();
      setTimeout(() => controller.abort(), 3000);
      const garminUrl = CONFIG.WORKERS?.['garmin-sync']?.replace('/health', '') || '';
      if (!garminUrl) return null;
      const res = await fetch(`${garminUrl}/status`, { signal: controller.signal });
